...
一个叫做 ngMin 的工具允许我们不用再去明确的定义所需要的依赖。ngMin 是为 Angular 应用做代码最小化之前预先做处理的工具。它会表里 Angular 应用的代码并为我们将注入依赖设置好。
...
ngMin 的核心工作原理是使用了抽象语法树来遍历 JavaScript 源文件。使用一个叫做 astral 的 AST 工具框架，ngMin 使用所需的标记重建源代码，最后使用 escodegen 将新的源代码输出。
